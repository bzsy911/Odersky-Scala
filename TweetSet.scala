...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = this.remove(elem).mostRetweetedAcc(elem)
  def mostRetweetedAcc(tweet: Tweet): Tweet = {
    if (elem.retweets > tweet.retweets) this.remove(elem).mostRetweetedAcc(elem)
...
object GoogleVsApple {
  ...
  def extractTweets(l: List[String], acc: TweetSet): TweetSet = {
    if (l.isEmpty) acc
    else extractTweets(l.tail, acc.union(tweets.filter(x => x.text.toUpperCase.contains(l.head.toUpperCase))))
  }

  lazy val googleTweets: TweetSet = extractTweets(google, new Empty)
  lazy val appleTweets: TweetSet = extractTweets(apple, new Empty)
