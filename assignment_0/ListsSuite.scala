...
class ListsSuite extends FunSuite {
  ...
  test("List consisting negatives and duplicates") {
    assert(sum(List(-3, -3, 20, -9)) === 5)
  }
  ...
  test("max for negatives and duplicates") {
    assert(max(List(7, -5, 7, 13, -8)) === 13)
  }
  
  test("an incresing List") {
    assert(max(List(-2, 0, 2, 4, 6)) === 6)
  }
}
